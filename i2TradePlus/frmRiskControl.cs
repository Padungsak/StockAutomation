...
namespace i2TradePlus
{
	public class frmRiskControl : Form
	{
		...
		private string GetSideString()
		{
			...
			{
				if (this.cbSide.Text.ToLower() == "sell")
				{
					result = "S";
				}
			}
		...
		private bool IsNotNull()
		{
			...
			{
				if (this.tbVolume.Text == string.Empty && this.tbPrice.Text == string.Empty)
				{
				...
				{
					if (this.tbVolume.Text != string.Empty && this.cbSide.Text == string.Empty)
					{
		...
		private void intzaTableGrid1_TableMouseClick(object sender, TableMouseEventArgs e)
		{
			try
			{
				...
				{
					if (e.RowIndex > -1 && e.RowIndex <= this.intza.Rows - 1)
					{
						this._isUpdate = true;
						...
						{
							if (recordItem.Fields("side").Text.ToString() == "S")
							{
		...
		private void InitializeComponent()
		{
			...
			this.intza.ScrollChennelColor = Color.LightGray;
