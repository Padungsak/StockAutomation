...
namespace i2TradePlus.FixForm
{
	public class frmStockRanking : ClientBaseForm, IRealtimeMessage
	{
		...
		private void InitializeComponent()
		{
			...
			this.intzaBestOpen.BackColor = Color.FromArgb(10, 10, 10);
			...
			this.intzaBestOpen.RowSelectColor = Color.FromArgb(64, 64, 64);
			this.intzaBestOpen.RowSelectType = 2;
			this.intzaBestOpen.RowsVisible = 40;
			this.intzaBestOpen.ScrollChennelColor = Color.Gray;
			...
			this.intzaProjectedOpen.BackColor = Color.FromArgb(10, 10, 10);
			...
			this.intzaProjectedOpen.RowSelectColor = Color.FromArgb(64, 64, 64);
			this.intzaProjectedOpen.RowSelectType = 2;
			this.intzaProjectedOpen.RowsVisible = 40;
			this.intzaProjectedOpen.ScrollChennelColor = Color.Gray;
			...
			this.intzaProjectedClose.BackColor = Color.FromArgb(10, 10, 10);
			...
			this.intzaProjectedClose.RowSelectColor = Color.FromArgb(64, 64, 64);
			this.intzaProjectedClose.RowSelectType = 2;
			this.intzaProjectedClose.RowsVisible = 40;
			this.intzaProjectedClose.ScrollChennelColor = Color.Gray;
			...
			this.intzaTopActive.BackColor = Color.FromArgb(10, 10, 10);
			...
			this.intzaTopActive.RowSelectColor = Color.FromArgb(64, 64, 64);
			this.intzaTopActive.RowSelectType = 2;
			this.intzaTopActive.RowsVisible = 40;
			this.intzaTopActive.ScrollChennelColor = Color.Gray;
		...
		private void ReloadDataPage(string page)
		{
			...
			{
				if (page == "Best Projected Open")
				{
					this.BestPOpen_ReloadData();
				}
				else
				{
					if (page == "Best Projected Close")
					{
						this.BestPClose_ReloadData();
					}
					else
					{
						if (page == "Best Open Price")
						{
		...
		private void TopActiveUpdateRealTimes(IBroadcastMessage message, StockList.StockInformation realtimeStockInfo)
		{
			...
			else
			{
				try
				{
					...
					{
						if (!this.TopActiveVerifyBoard(realtimeStockInfo))
						{
							return;
						}
					}
		...
		private void BestOpen_ReloadData()
		{
			try
			{
				...
				{
					if (this.openSessionBestOpenPrice == 2)
					{
						...
						else
						{
							...
							else
							{
								if (this.currentPage == "Best Open Price")
								{
									if (realtimeStockInfo != null)
									{
										if (!(realtimeStockInfo.MarketId != this.marketIdBestOpenPrice))
										{
											if (this.boardTypeBestOpenPrice == frmStockRanking.BoardTypes.M)
											{
												if (realtimeStockInfo.StockBoard != "M")
											...
											{
												LSAccumulate lSAccumulate = (LSAccumulate)message;
												int num = this.intzaBestOpen.FindIndex("stock", realtimeStockInfo.Symbol);
												if (num > -1)
												{
													long openVolume2 = -1L;
													decimal openPrice = 0m;
													decimal priorOrBreakclose = 0m;
													if (lSAccumulate.Side == string.Empty)
													{
														openVolume2 = lSAccumulate.Volume * (long)realtimeStockInfo.BoardLot;
														openPrice = lSAccumulate.LastPrice;
														decimal.TryParse(this.intzaBestOpen.Records(num).Fields("prior").Text.ToString(), out priorOrBreakclose);
													}
													decimal avg = 0m;
													if (lSAccumulate.AccVolume > 0L)
													{
													...
													this.BestOpen_UpdateByLS(num, avg, openVolume2, realtimeStockInfo.HighPrice, realtimeStockInfo.LowPrice, lSAccumulate.LastPrice, openPrice, priorOrBreakclose, realtimeStockInfo);
													if (base.IsAllowRender)
													{
														this.intzaBestOpen.EndUpdate(num);
													}
												}
											}
										}
